...
public class MedMedFirstOccIndexDeltaTreeHeuristic implements HeuristicStrategy<PlaceNode, TreeNodeScore>, ZeroOneBounded, SubtreeMonotonicity.Decreasing {
    ...
    public MedMedFirstOccIndexDeltaTreeHeuristic(Map<Activity, Double> activityToMedFirstOccurrenceIndex, BidiMap<Activity, Transition> actTransMapping, double alpha, double maxDelta, int maxSize) {
    ...
    public static class Builder extends ComponentSystemAwareBuilder<MedMedFirstOccIndexDeltaTreeHeuristic> {
        ...
        @Override
        protected MedMedFirstOccIndexDeltaTreeHeuristic buildIfFullySatisfied() {
            ...
            return new MedMedFirstOccIndexDeltaTreeHeuristic(activityToMedFirstOccurrenceIndex, actTransMapping.getData(), alpha.getData().getP(), maxDelta, maxSize);
        }
    }
    ...
    public TreeNodeScore computeHeuristic(PlaceNode node) {
        ...
        for (Transition t : p.preset()) {
            ...
            avgIndicesPreset[i] = activityToMedFirstOccurrenceIndex.get(a);
            i++;
        }
        ...
        for (Transition t : p.postset()) {
            Activity a = actTransMapping.getKey(t);
            avgIndicesPostset[i] = activityToMedFirstOccurrenceIndex.get(a);
            i++;
        }
