...
public class MeanModFirstOccIndexDeltaTreeHeuristic implements HeuristicStrategy<PlaceNode, TreeNodeScore>, ZeroOneBounded, SubtreeMonotonicity.Decreasing {
    ...
    public MeanModFirstOccIndexDeltaTreeHeuristic(Map<Activity, Integer> activityToModFirstOccurrenceIndex, BidiMap<Activity, Transition> actTransMapping, double alpha, double maxDelta, int maxSize) {
    ...
    public static class Builder extends ComponentSystemAwareBuilder<MeanModFirstOccIndexDeltaTreeHeuristic> {
        ...
        @Override
        protected MeanModFirstOccIndexDeltaTreeHeuristic buildIfFullySatisfied() {
            ...
            return new MeanModFirstOccIndexDeltaTreeHeuristic(activityToModFirstOccurrenceIndex, actTransMapping.getData(), alpha.getData().getP(), maxDelta, maxSize);
        }
    }
    ...
    public TreeNodeScore computeHeuristic(PlaceNode node) {
        ...
        for (Transition t : p.preset()) {
            ...
            avgIndexPreset += activityToModFirstOccurrenceIndex.get(a);
        }
        ...
        for (Transition t : p.postset()) {
            Activity a = actTransMapping.getKey(t);
            avgIndexPostset += activityToModFirstOccurrenceIndex.get(a);
        }
